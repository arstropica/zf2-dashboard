...
HTML;
	...
	{
		$TR = '<tr data-toggle="collapse" data-target="#data%2$s" class="accordion-toggle %5$s">%3$s</tr>' . '<tr class="hiddenRow"><td colspan="%1$d">' . '<div class="accordion-body collapse" id="data%2$s">%4$s</div>' . '</td></tr>';
		$TD = '<td>%s</td>';
		$TDs = sprintf($TD, $index + 1) . implode("\n", array_map(function ($h) use($data, $TD) {
			$v = "N/A";
			if (isset($data [$h])) {
		...
		$TABLE = '<table class="table table-condensed table-striped" class="margin: 0px;">' . implode("\n", array_map(function ($d, $k) use($TD) {
			...
			if (is_array($v)) {
				...
				foreach ( $d as $kv ) {
					$k = key($kv);
					$v = current($kv);
					$output [] = sprintf('<tr>' . $TD . $TD . '</tr>', $k, $v);
				}
				return implode("\n", $output);
			} else {
