...
class FormRow extends FormRowHelper
{
	...
	public function render (ElementInterface $oElement, $labelPosition = null)
	{
		...
		if (($sColumSize = $oElement->getOption('column-size')) &&
				 $sLayout !== FormHelper::LAYOUT_HORIZONTAL) {
		...
		if (($sElementType === 'submit' || $sElementType === 'button' ||
				 $sElementType === 'reset') &&
				 $sLayout === FormHelper::LAYOUT_INLINE) {
			return $sElementContent . PHP_EOL;
		}
		...
		if (($sLabelContent = $this->renderLabel($oElement)) != false) {
			...
			if (($oElement instanceof Button) or ($oElement instanceof Submit)) {
				...
				switch ($sLayout) {
					...
					case FormHelper::LAYOUT_HORIZONTAL:
						if ($sElementType !== 'checkbox') {
							if (empty($aLabelAttributes['class'])) {
								$aLabelAttributes['class'] = 'control-label';
							} else {
