...
class ImportController extends AbstractCrudController {
	...
	public function importAction()
	{
		...
		$view->get('HeadLink')
			->appendStylesheet($view->get('basePath')
			->__invoke('css/nav-wizard.bootstrap.css'));
		
		$view->get('HeadScript')
			->appendFile($view->get('basePath')
	...
	protected function handleUpload($post, $form, $reload = false)
	{
		...
		$files = $request->getFiles()
			->toArray();
		$data = array_merge_recursive($post, $request->getFiles()
			->toArray());
		...
		if ($form->isValid()) {
			...
			if ($messages) {
				$this->flashMessenger()
					->addErrorMessage($this->formatFormMessages($form));
			}
		}
		return $outcome;
	}
		...
		if (!$required) {
			...
			if ($data) {
				...
				if (count($invalid) && !$overrides) {
					$this->flashMessenger()
						->addErrorMessage(count($invalid) . " potential duplicate leads were found in your imported data.");
				}
			}
		}
		return $valid;
...
JTPL;
	...
	protected function parseFile($file)
	{
		...
		try {
			...
			$first_row = $worksheet->getRowIterator(2)
				->current();
			if ($first_row && $first_row instanceof \PHPExcel_Worksheet_Row) {
				/* @var $rcell \PHPExcel_Cell */
				foreach ( $first_row->getCellIterator() as $rcell ) {
					$format = $rcell->getStyle()
						->getNumberFormat()
						->getFormatCode();
					if ($format == 'mm-dd-yy') {
						$c = $rcell->getColumn();
						$column = $worksheet->getColumnIterator($c)
							->current();
						
						if ($column && $column instanceof \PHPExcel_Worksheet_Column) {
							/* @var $ccell \PHPExcel_Cell */
							foreach ( $column->getCellIterator() as $ccell ) {
								if ($ccell->getDataType() == 'n') {
									$ccell->getStyle()
										->getNumberFormat()
			...
			$rows = $objPHPExcel->getActiveSheet()
				->toArray();
		} catch ( \Exception $e ) {
			$this->flashMessenger()
				->addErrorMessage($e->getMessage());
		...
		if ($rows && count($rows) > 1) {
			...
			if ($row_count !== $i) {
				$error_message = ($row_count - $i) . " of " . $row_count . " records could not be analyzed. Do you have missing data?";
				$this->flashMessenger()
	...
	protected function checkDuplicateImport($row, $index)
	{
		...
		if ($key_values) {
			if ($lead) {
				...
				$data = $this->data;
				if ($data) {
					// Find potential duplicate values and filter them by key
					// values
	...
	protected function calculateSimilarity($data, $key_values, $row = [])
	{
		...
		$objRepository = $this->getEntityManager()
			->getRepository('\Lead\Entity\LeadAttribute');
		
		// Assign scores to records based on the number and type of matching key
		// values
