 ...
 *        
 */
class PostClientDataService extends AbstractTenStreetService implements EventManagerAwareInterface {
	 ...
	 */
	public function send($id, $service = "TenStreet")
	{
		$this->getServiceEvent()
			->setEntityId($id)
			->setEntityClass('Lead\Entity\Lead')
			->setDescription('TenStreet Submission');
		...
		if (isset($clientId, $password, $service)) {
			...
			try {
				$response = $this->PostClientData($id, $xml_data, $clientId, $password, $service);
				$result = $this->parse($response);
			} catch ( \Exception $e ) {
				return $this->respondError($e);
			}
		} else {
			return $this->respondError(new \Exception('Insufficient API Authorization.', 401));
		}
	 ...
	 */
	public function getOptions($id, $service = "TenStreet")
	{
		$options = [ ];
		$credentials = $this->getTenStreetDataMapper()
			->getCredentials();
	...
	public function getData($id)
	{
		...
		$tenStreetData = $this->getTenStreetDataMapper()
			->getById($id);
		...
		try {
			$data = $this->getServiceLocator()
				->get('HydratorManager')
				->get('TenStreet\Hydrator\TenStreetDataHydrator')
	...
	{
		$this->getServiceEvent()
			->setMessage($result ['lastresponse'] ?  : 'Unknown Response')
			->setOutcome($result ['submitted'] ?  : 0);
		$this->logEvent('PostClientData.post');
		return $this->respond($this->errorResponse->successOperation($result));
	}

	public function PostClientData($id, $xml_data, $clientId, $password, $service)
	{
		$xml_response = null;
		
		try {
			$xml_response = $this->getClient()
				->PostClientData($xml_data, $clientId, $password, $service);
		} catch ( \SoapFault $e ) {
