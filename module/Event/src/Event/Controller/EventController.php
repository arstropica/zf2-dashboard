...
class EventController extends AbstractCrudController
{
	...
	public function listAction ()
	{
		...
		$paginator = new Paginator(
				new DoctrinePaginator(new ORMPaginator($qb->getQuery(), true)));
	...
	public function viewAction ()
	{
		...
		/* @var $qb \Doctrine\ORM\QueryBuilder */
		$qb = $this->getEntityManager()->createQueryBuilder();
		$qb->add('select', 'e,' . implode(', ', array_keys($baseClasses)))->add(
				'from', $this->getEntityClass() . ' e');
		foreach ($baseClasses as $alias => $prefix) {
			$qb->leftJoin("Event\\Entity\\{$prefix}Event", $alias, 'WITH', 
					"e = {$alias}.event");
		}
	...
	protected function getFilterForm ($data = array())
	{
		...
		$form = $sl->get('FormElementManager')->get('Event\Form\FilterForm');
			...
			foreach ($filters as $condition) {
				if (isset($query[$condition]) && "" !== $query[$condition]) {
					switch ($condition) {
						...
						case 'account':
							switch ($query[$condition]) {
								default:
									...
									$qb->innerJoin('Event\Entity\AccountEvent', 
											'ae', 'WITH', 'e = ae.event')->innerJoin(
											'ae.account', 'account');
						...
						case 'event':
							$qb->innerJoin(
									"Event\\Entity\\{$query[$condition]}", 'v', 
									'WITH', 'e = v.event');
							break;
					}
				}
